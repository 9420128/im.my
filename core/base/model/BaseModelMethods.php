        ...
        if(!isset($set['fields']) || !is_array($set['fields']) || !$set['fields']){
            if(!$join){
                ...
                foreach ($this->tableRows[$alias_table] as $key => $item){
                    ...
                    if($key !== 'id_row' && $key !== 'multi_id_row'){
                        $fields .= $concat_tale . $key . ' as TABLE' . $alias_table . 'TABLE_' . $key . ',';
                    }
                }
            }
        }
                ...
                if($field){
                    ...
                    if($join && $join_structure){
                        ...
                        if(preg_match('/^(.+)?\s+as\s+(.+)/i', $field, $matches)){
                            ...
                            $fields .= $concat_tale . $matches[1] . ' as TABLE' . $alias_table . 'TABLE_' . $matches[2] . ',';

                        }
                        else{

                            $fields .= $concat_tale . $field . ' as TABLE' . $alias_table . 'TABLE_' . $field . ',';

                        }
                    }
                    else{

                        $fields .= $concat_tale . $field . ',';
            ...
            if(!$id_field && $join_structure){
                ...
                if($join){
                    ...
                    $fields .= $concat_tale . $this->tableRows[$alias_table]['id_row'] . ' as TABLE' . $alias_table . 'TABLE_' . $this->tableRows[$alias_table]['id_row'] . ',';

                }
                else{

                    $fields .= $concat_tale . $this->tableRows[$alias_table]['id_row'] . ',';
